...
`<nav data-ns-isle>` with `[data-ns-isle-toggle]` (+ `-icon`, `-label`, `-pos`) and a `[data-ns-isle-panel]` sheet of section links (`[data-ns-isle-close]`, `[data-ns-isle-handle]`); `isle(el, { pos, onChange })` → `{ open(), close(), toggle(), go(i), index, destroy() }`. Tracks the current section with IntersectionObserver, stays full while scrolling down and collapses to an icon on scroll up (`collapseOn: 'down'` inverts it), swipe = previous/next section. Tapping morphs the capsule into the sheet like Apple's Dynamic Island: it widens first, then grows upward (one spring per axis), with the sheet's real content appearing piece by piece as the shape reaches it; an opaque 7-piece (9-slice) silhouette animates only transform/opacity along a precomputed path (compositor thread; Safari does not composite clip-path), then the glass material fades in on top (`morph: false` = slide up; `--ns-isle-radius` 32px, `--ns-isle-morph` color). Page scroll is locked while the sheet is open, including iOS touch. Uses `ns-frame/sheet`. Full options in `docs/componentes.md`.
