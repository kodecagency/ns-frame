...
function place(p) {
  ...
  const base = p.getAttribute('data-ns-arrow') || 'all round 8'
  const shape = `${base}; ${side} tab ${(x - D).toFixed(1)} ${(x + D).toFixed(1)} ${D} ${D}`
...
function init(p) {
  ...
  p.addEventListener('beforetoggle', e => {
    if (e.newState == 'open') { OPEN.add(p); p.setAttribute('data-ns', (p.getAttribute('data-ns-arrow') || 'all round 8') + '; bottom tab center 18 9 9') }
    else OPEN.delete(p)
  })
...
function tip(t) {
  ...
  let timer
  const show = () => { clearTimeout(timer); timer = setTimeout(() => { p._nsT = t; p.matches(':popover-open') || p.showPopover() }, 120) }
  const hide = () => { clearTimeout(timer); timer = setTimeout(() => p.matches(':popover-open') && p.hidePopover(), 80) }
  t.addEventListener('pointerenter', show); t.addEventListener('focus', show)
  t.addEventListener('pointerleave', hide); t.addEventListener('blur', hide)
